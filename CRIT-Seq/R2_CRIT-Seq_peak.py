...
class chromosomePosition():
    ...
    def SummarizeBarcodeIndex(self):
        ...
        for index in self.index_stack:
            ...
            offtarget_sequence = self.getSequence(self.genome, most_frequent_chromosome, most_frequent_position - 35,
                                                  most_frequent_position + 35)

            summary_list = [str(x) for x in
                            [index, most_frequent_chromosome, most_frequent_position, offtarget_sequence,
                             # pick most frequently occurring chromosome and position
...
def parseReadName(read_name):
    m = re.search(r'([ACGTN]{10})_([0-9]*)', read_name)
    if m:
...
def alignSequences(ref_seq, query_seq):
    ...
    matches_required = len(ref_seq) - 1 - 4  # allow up to 8 mismatches
    scoring = swalign.NucleotideScoringMatrix(match, mismatch)
    sw = swalign.LocalAlignment(scoring, gap_penalty=-100, gap_extension_penalty=-100,
                                prefer_gap_runs=True)  # you can also choose gap penalties, etc...
    ...
    if forward_alignment.matches >= matches_required and forward_alignment.matches > reverse_alignment.matches:
        ...
        strand = "+"
        return [forward_alignment.query[start:end], ref_length - forward_alignment.matches - 1, end - start, strand,
                start, end]
        ...
        strand = "-"
        return [reverse_alignment.query[start:end], ref_length - reverse_alignment.matches - 1, end - start, strand,
                start, end]
    else:
        return ["", "", "", "", "", ""]
...
def analyze(sam_filename, reference_genome, outfile, annotations):
    ...
    for line in file:
        ...
        if not bb:
            ...
            full_read_name, sam_flag, chromosome, position, mapq, cigar, name_of_mate, position_of_mate, template_length, read_sequence, read_quality = fields[
                                                                                                                                                        :11]
            ...
            if int(template_length) < 0:  # Reverse read
                ...
                strand = "-"
                # print(chromosome, read_position, strand, barcode, primer, count)
    ...
    with open(outfile, 'w') as f:
        ...
        for row in summary:
            ...
            if target_sequence:
                sequence, mismatches, length, strand, target_start_relative, target_end_relative = alignSequences(
                    target_sequence, window_sequence)
                ...
                f.write('\t'.join(row[4:8] + [filename_tail] + row[0:4] + row[8:] +
                                  [str(x) for x in [sequence, mismatches, length, BED_chromosome, target_start_absolute,
                                                    target_end_absolute, BED_name, BED_score, strand]] + [str(x) for x
                                                                                                          in
                                                                                                          annotation] + [
                                      '\n']))
            else:
                ...
                f.write('\t'.join(
                    [str(x) for x in row[4:8] + [filename_tail] + row[0:4] + row[8:] + [""] * 9 + annotation] + ['\n']))


def main():
    if len(sys.argv) < 6:
        print(
            'Usage: python consolidate.py sam_filename, reference_genome, outfile, annotations[[Description,Targetsite,Squence]]')
        sys.exit()
    sam_filename = sys.argv[1]
    reference_genome = sys.argv[2]
    outfile = sys.argv[3]
    annotations = {}
    annotations['Description'] = str(sys.argv[4])
    annotations['Targetsite'] = str(sys.argv[5])
    annotations['Sequence'] = str(sys.argv[6])
    analyze(sam_filename, reference_genome, outfile, annotations)
